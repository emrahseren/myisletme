...
function UpdateUserDetails() {
    ...
    var pass1 = $("#update_password1").val();
    var pass2 = $("#update_password2").val();
    
    if(pass1 != "" || pass1 != null || pass1 != pass2){
        $('.update_mm').html('<div class="alert alert-warning">Password Wrong!!</div>');            
            $(".update_mm").delay(300).show(10, function() {
                    $(this).delay(2000).hide(10);});
    }
    else {
    ...
    $.post("user_controller.php?a=updateUser", {
        ...
        function (data, status) {
            //$find(".update_modal").hide();
            
            if(status == 'success')
            $('#update_modal').delay(1000).fadeOut(200, function(){ 
